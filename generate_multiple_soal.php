...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
     // Setup API Groq
     $apiKey = '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';  // Ganti dengan API key Anda
     ...
     if($httpCode != 200){
        ...
        if($tipe_soal == 'pilihan_ganda') {
            ...
            foreach($soal_array as $index => $soal_text) {
                ...
                if(preg_match('/Soal:\s*(.*?)(?=A\.|$)/s', $soal_text, $pertanyaan) &&
                ...
                if(empty($pertanyaan[1]) || empty($jawaban_a[1]) || empty($jawaban_b[1]) || 
