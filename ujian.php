...
<script>
    ...
    examCards.forEach((card) => {
        ...
        let examId = '';
        const linkElement = card.querySelector('a[href^="buat_soal.php?ujian_id="]') || 
                            card.querySelector('a[href^="mulai_ujian.php?id="]');

        if (linkElement) {
            const href = linkElement.getAttribute('href');
            const match = href.match(/ujian_id=(\d+)/) || href.match(/id=(\d+)/);
            if (match && match[1]) {
                examId = match[1];
            }
        }

        // Get exam start date and end date (from database data in examMapping)
        let examData = examMapping[examId] || {};
        
        // Get start date from card
        let startDate = null;
        const dateElements = card.querySelectorAll('.d-flex.align-items-center');
        ...
        dateElements.forEach(el => {
            ...
            if (icon && icon.classList.contains('bi-calendar-event')) {
                const textElement = el.querySelector('.text-secondary');
                if (textElement) {
                    startDate = parseExamDate(textElement.textContent.trim());
                }
            }
        });
        
        // Get the exam status to determine if it's active today
        let isActiveToday = false;
        const statusButton = card.querySelector('.btn');
        if (statusButton) {
            const buttonText = statusButton.textContent.trim().toLowerCase();
            if (buttonText.includes('mulai ujian')) {
                isActiveToday = true; // This is an active exam
            }
        }
        ...
        } else if (filter === 'past' && startDate) {
            // Show exams that have completely ended
            const isCompleted = !isActiveToday && startDate < today;
            parentCol.style.display = isCompleted ? 'block' : 'none';
        } else if (filter === 'today') {
            // Show exams that are active today - either via button status or date logic
            parentCol.style.display = isActiveToday ? 'block' : 'none';
        } else if (filter === 'upcoming' && startDate) {
            // Show future exams
            parentCol.style.display = (startDate > today && !isActiveToday) ? 'block' : 'none';
